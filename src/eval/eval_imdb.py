...
def main():
    ...
    ds = load_dataset("imdb", cache_dir=args.cache_dir)
    ...
    for seed in seeds:
        ...
        # tokenize (batched) + keep only needed columns
        ds_tok = ds.map(lambda batch: tokenize(batch, tok), batched=True)
        ds_tok = ds_tok.remove_columns(
            [c for c in ds_tok["train"].column_names if c not in ["input_ids", "attention_mask", "label"]]
        )
        ...
        # ✅ ВОТ ЭТО ЧИНИТ КРАШ: padding до max в батче
    ...
    med = median(runs)
    save_json(
        {"model": args.model, "seeds": seeds, "runs": runs, "median": med},
        os.path.join(args.out_dir, "imdb.json"),
    )

    print_table(
        "TABLE 2 (IMDb test accuracy)",
        ["model", "median_acc", "runs"],
        [[args.model, f"{med:.4f}", str([round(x, 4) for x in runs])]],
    )
