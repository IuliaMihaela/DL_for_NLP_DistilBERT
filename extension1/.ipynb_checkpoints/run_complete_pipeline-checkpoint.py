...
class CompletePipeline:
    ...
    def generate_presentation_summary(self):
        summary_path = os.path.join(self.output_dir, "presentation_summary.txt")
        
        baseline = self.all_results['baseline']
        combined = self.all_results['combined']
        
        size_reduction = ((baseline['model_size_mb'] - combined['model_size_mb']) / baseline['model_size_mb']) * 100
        
        baseline_latency = [v for k, v in baseline.items() if 'latency' in k][0]
        combined_latency = [v for k, v in combined.items() if 'latency' in k][0]
        speedup = ((baseline_latency - combined_latency) / baseline_latency) * 100
...
BASELINE (After Triple Loss Training):
  - Model Size: {baseline['model_size_mb']:.2f} MB
  - Parameters: {baseline['total_params']:,}
  - Inference Time: {baseline_latency:.2f} ms
  - Throughput: {baseline['throughput_samples_per_sec']:.2f} samples/sec

OPTIMIZED (Pruning + Quantization):
  - Model Size: {combined['model_size_mb']:.2f} MB
  - Parameters: {combined['total_params']:,}
  - Inference Time: {combined_latency:.2f} ms
  - Throughput: {combined['throughput_samples_per_sec']:.2f} samples/sec

IMPROVEMENTS:
  Size Reduction: {size_reduction:.1f}%
  Speed Improvement: {speedup:.1f}%
  Suitable for edge deployment: {'YES' if combined['model_size_mb'] < 50 else 'NEEDS MORE OPTIMIZATION'}
...
3. Our optimizations reduced size by {size_reduction:.1f}% to {combined['model_size_mb']:.2f}MB
4. Inference improved by {speedup:.1f}%
...
{'='*70}
        ...
        print(f"\nPresentation summary saved to {summary_path}")
        print(summary)
